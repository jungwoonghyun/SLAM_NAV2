...
class PacketESP32RobotSwitch : public rclcpp::Node
{
public:
  // Node 클래스의 생성자를 호출, 노드 이름을 packet_RS485_publisher 지정, count_는 0으로 초기화
  PacketESP32RobotSwitch()
  : Node("esp32_robot_switch")
  {
    ...
    packet_ESP32_Subscriber_ = this->create_subscription<message_interface::msg::Ledcomm>(
      "led_instructor",
      qos_profile,
      std::bind(&PacketESP32RobotSwitch::state_message, this, _1));
    ...
    timer_ = this->create_wall_timer(
      50ms, std::bind(&PacketESP32RobotSwitch::esp32_msg, this));
  }
private:
  ...
  void esp32_msg()
  {
    ...
    // joystick
    buttonData.button[0] = (int)packet_buf[6]; // joystick_rl

    buttonData.button[1] = (int)packet_buf[7]; // joystick_fb


    // mod button
    buttonData.button[4] = !(packet_buf[8] / 4); // mod (follow)
    cal_data = packet_buf[8] % 4;

    buttonData.button[5] = !(cal_data / 2); // mod (replay)
    cal_data = packet_buf[8] % 2;

    buttonData.button[6] = !(cal_data / 1); // mod (playback)


    // up/down & joystick center & headright button
    buttonData.button[7] = !(packet_buf[11] / 64); // headlight on/off -> shutdown

    buttonData.button[9] = !(packet_buf[9] / 2); // up
    cal_data = packet_buf[9] % 2;

    buttonData.button[8] = !(cal_data / 1); // down

    buttonData.button[10] = !((packet_buf[10] % 2) / 1); // joystick center
    ...
    printf("write button data : %3d %3d | %2d %2d %2d | %2d %2d %2d %2d\r\n",
          buttonData.button[15], buttonData.button[14], buttonData.button[13],
          buttonData.button[12], buttonData.button[11], buttonData.button[10],
          buttonData.button[9], buttonData.button[8], buttonData.button[7]);
...
int main(int argc, char * argv[])
{
  ...
  // 클래스 생성
  auto node = std::make_shared<PacketESP32RobotSwitch>();
