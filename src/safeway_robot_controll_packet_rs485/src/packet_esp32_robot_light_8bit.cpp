...
class PacketESP32RobotLight8bit : public rclcpp::Node
{
public:
  // Node 클래스의 생성자를 호출, 노드 이름을 packet_RS485_publisher 지정, count_는 0으로 초기화
  PacketESP32RobotLight8bit()
  : Node("esp32_robot_light_8bit")
  {
    ...
    packet_ESP32_Subscriber_ = this->create_subscription<message_interface::msg::Ledcomm>(
      "led_instructor",
      qos_profile,
      std::bind(&PacketESP32RobotLight8bit::state_message, this, _1));

    // 콜백 함수를 수행, 지정한 시간마다 지정한 콜백함수를 실행
    timer_ = this->create_wall_timer(
      50ms, std::bind(&PacketESP32RobotLight8bit::esp32_msg, this));
  }
private:
  ...
  void esp32_msg()
  {
    ...
    g_rw_error[0] = read(fd, read_buf, 8);
    g_rw_error[1] = 1;
    ...
    write_buf[3] = 0x00;
    write_buf[4] = (uint8_t)(!light_comm[7]) * 128 + (uint8_t)(!light_comm[6]) * 64 + (uint8_t)(!light_comm[5]) * 32 + (uint8_t)(!light_comm[4]) * 16
                + (uint8_t)(!light_comm[3]) * 8 + (uint8_t)(!light_comm[2]) * 4 + (uint8_t)(!light_comm[1]) * 2 + (uint8_t)(!light_comm[0]) * 1; // LED State
...
int main(int argc, char * argv[])
{
  ...
  // 클래스 생성
  auto node = std::make_shared<PacketESP32RobotLight8bit>();
