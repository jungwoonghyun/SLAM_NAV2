...
class PacketESP32RobotSwitch8bit : public rclcpp::Node
{
public:
  // Node 클래스의 생성자를 호출, 노드 이름을 packet_RS485_publisher 지정, count_는 0으로 초기화
  PacketESP32RobotSwitch8bit()
  : Node("esp32_robot_switch_8bit")
  {
    ...
    packet_ESP32_Subscriber_ = this->create_subscription<message_interface::msg::Ledcomm>(
      "led_instructor",
      qos_profile,
      std::bind(&PacketESP32RobotSwitch8bit::state_message, this, _1));
    ...
    timer_ = this->create_wall_timer(
      50ms, std::bind(&PacketESP32RobotSwitch8bit::esp32_msg, this));
  }
private:
  ...
  void esp32_msg()
  {
    ...
    if(error_code > 0) // error state : read before packet
    {
      ...
      printf("Read packet error : %2x %2x | %2x %2x %2x | %2x %2x | %2x\r\n",
            ...
            (uint16_t)packet_buf[6], (uint16_t)packet_buf[7]);
    ...
    else
    {
      ...
      printf("Read packet success : %2x %2x | %2x %2x %2x | %2x %2x | %2x\r\n",
            (uint16_t)packet_buf[0], (uint16_t)packet_buf[1], (uint16_t)packet_buf[2],
            (uint16_t)packet_buf[3], (uint16_t)packet_buf[4], (uint16_t)packet_buf[5],
            (uint16_t)packet_buf[6], (uint16_t)packet_buf[7]);
    }
    ...
    // joystick
    buttonData.button[0] = (int)packet_buf[5]; // joystick_rl

    buttonData.button[1] = (int)packet_buf[6]; // joystick_fb

    // mod button
    buttonData.button[4] = !(packet_buf[2] / 16); // mod (follow)
    cal_data = packet_buf[2] % 16;

    buttonData.button[5] = !(cal_data / 8); // mod (replay)
    cal_data = packet_buf[2] % 8;

    buttonData.button[6] = !(cal_data / 4); // mod (playback)
    cal_data = packet_buf[2] % 4;

    // up/down & joystick center & headright button
    buttonData.button[9] = !(cal_data / 2); // up
    cal_data = packet_buf[2] % 2;

    buttonData.button[8] = !(cal_data / 1); // down

    buttonData.button[7] = !(packet_buf[4] / 64); // headlight on/off -> shutdown

    buttonData.button[10] = !((packet_buf[3] % 2) / 1); // joystick center


    // non-used button data
    buttonData.button[2] = 0;

    buttonData.button[3] = 0;

    buttonData.button[11] = 0;

    buttonData.button[12] = 0;

    buttonData.button[13] = 0;

    buttonData.button[14] = 0;

    buttonData.button[15] = 0;

    printf("write button data : %3d %3d | %2d %2d %2d | %2d %2d %2d %2d\r\n",
          buttonData.button[0], buttonData.button[1],
          buttonData.button[4], buttonData.button[5], buttonData.button[6],
          buttonData.button[7], buttonData.button[9], buttonData.button[8], buttonData.button[10]);
...
int main(int argc, char * argv[])
{
  ...
  // 클래스 생성
  auto node = std::make_shared<PacketESP32RobotSwitch8bit>();
