...
int main(int argc, char *argv[]) {
  ...
  std::string frame_id = "base_scan"; // laser_frame
  ...
  auto laser_pub = node->create_publisher<sensor_msgs::msg::LaserScan>("scan", 10); // rclcpp::SensorDataQoS() -> 30
  auto pc_pub = node->create_publisher<sensor_msgs::msg::PointCloud>("point_cloud", 10); // rclcpp::SensorDataQoS() -> 30
  ...
  rclcpp::WallRate loop_rate(10); // loop_rate(10);
  ...
  while (ret && rclcpp::ok()) {
    ...
    // std::cout << "x distance, " << scan.points[scan.points.size()*5/12].range << "," << std::put_time(&now_tm, "%Y-%m-%d %H %M %S,                 ")
    // << std::setw(3) << std::setfill('0') << now_ms.count() << "\n";
    // std::cout << "x distance, " << scan.points[scan.points.size()*5/12].range << "\n";
